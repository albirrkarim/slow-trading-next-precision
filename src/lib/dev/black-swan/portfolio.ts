...
import exit, {
  type ExitEvaluationConfig,
} from "@/lib/system/trading/exit";
...
const BACKTEST_ONE_SIDE_FEE_RATIO = 0.001;
...
function exitFromDecision(params: {
  ...
  const isLiquidated = params.reason === "LIQUIDATED";
  return {
...
/**
 * Hard leveraged-PnL floor ported from the legacy backtest exit policy: once a
 * position loses 80% of its leveraged margin it counts as liquidated.
 */
function isLiquidatedAt(params: {
  metrics: PositionMetrics;
  position: Position;
}): boolean {
  const leverage = Math.max(1, Number(params.position.exposure.leverage) || 1);
  const grossPct = calculatePnlPct(
    params.metrics.grossPnlUsdt,
    params.metrics.notionalUsdt,
  );
  return grossPct * leverage <= LIQUIDATION_LEVERAGED_PNL_PERCENT;
}
...
function applyAveraging(params: {
  ...
  const step = reserve.averaging.getNextWatchStep({
  ...
  const projection = reserve.averaging.resolveRescueProjection({
  ...
  const reservedBefore = reserve.state.getReservedRemainingUsdt(
    params.position.strategy.averaging,
  );
  const execution = {
    adaptiveMultiplier:
  ...
  const reservedAfter = reserve.state.getReservedRemainingUsdt(
    params.position.strategy.averaging,
  );
...
function replayPosition(params: {
  ...
  let reservedQuoteAsset = reserve.state.getReservedRemainingUsdt(
  ...
  const evaluationConfig = params.tradingConfig as ExitEvaluationConfig;
  const roundTripFeeRatio = params.oneSideFeeRatio * 2;

  const finish = (exitOutcome: ReplayExit): ReplayOutcome => {
    const snapshot = positionAt({
      exitPrice: exitOutcome.price,
      lots,
    ...
    snapshot.position.pnl = {
      ...snapshot.position.pnl,
      currentValueUsdt: snapshot.metrics.notionalUsdt + exitOutcome.pnlUsdt,
      markPrice: exitOutcome.price,
      netPct: exitOutcome.pnlPct,
      netUsdt: exitOutcome.pnlUsdt,
    };
    const confirmedVPoints = orderedVPoints
      .slice(0, vPointIndex)
      .map(({ point }) => point);
    const speedupReasons = positions.getSpeedupReasons({
    ...
    return {
      ...
      monitoringReasonAtExit:
        speedupReasons.length > 0
          ? positions.describeSpeedupReasons(speedupReasons)
          : positions.describeStandardReason({
              negativePnlThresholdPct:
                params.monitoringConfig?.negativePnlThresholdPct,
              positivePnlThresholdPct:
                params.monitoringConfig?.positivePnlThresholdPct,
  ...
  for (let candleIndex = 0; candleIndex < params.candles.length; candleIndex += 1) {
    ...
    let exited: ReplayOutcome | null = null;
    for (const currentPrice of prices) {
      ...
      if (decision.action === "SELL") {
        ...
        exited = finish(exitFromDecision({
          metrics: exitSnapshot.metrics,
          price: exitPrice,
          reason: decision.position?.closed?.reason ?? "UNKNOWN",
          t,
        }));
        break;
      }
      if (isLiquidatedAt({ metrics: snapshot.metrics, position })) {
        exited = finish(exitFromDecision({
  ...
  const finalDecision = exit.evaluate({
    config: evaluationConfig,
    position: snapshot.position,
  ...
  return finish(exitFromDecision({
    metrics: snapshot.metrics,
    price: finalDecision.position?.closed?.price ?? finalPrice.price,
    reason: finalDecision.position?.closed?.reason ?? "FINAL",
    t: finalPrice.t,
  }));
}
...
function buildFocusedVPoints(
  ...
  const exitIndex = findPointIndex(points, {
    id:
      exitOutcome.t === position.closed?.t
        ? position.closed?.vPoint?.id
        : undefined,
    t: Math.max(exitOutcome.t, protectionT),
  });
  return points
    .slice(
