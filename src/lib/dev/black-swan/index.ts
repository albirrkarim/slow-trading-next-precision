...
async function loadCandles(params: {
  ...
  const candles = await klines.downloadRange({
    closedOnly: true,
...
function buildVPointConfirmationTimes(params: {
  ...
  for (let index = 1; index < params.candles.length; index += 1) {
    ...
    const processed = vpoints.processKline({
      kline: candle,
      memory,
      previousPoint,
    ...
    if (processed.point) {
      confirmationTimes[processed.point.id] = Number(candle[6]);
      previousPoint = processed.point;
    }
  }
...
async function runSavings(
  ...
  const backtest = await runtimeQuickBacktest.run({
    config: {
      ...input.tradingConfig,
  ...
  const replayInput = {
    ...
    tradingMode: input.tradingConfig.tradingMode ?? TradingMode.FUTURES,
    volatilityMap,
