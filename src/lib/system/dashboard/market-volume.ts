...
import runtimePublicMarketCache from "../utils/public-market-cache";
import storageFiles from "../storage/files";

const VOLUME_24H_CACHE_TTL_MS = 10 * 60_000;

export type RuntimeVolumeMarketType = "FUTURES" | "SPOT";

export interface RuntimeVolume24hSnapshot {
  exchangeType: ExchangeType;
  marketType: RuntimeVolumeMarketType;
...
function snapshotPath(
  ...
  return storageFiles.prod.cache.ticker24h(exchangeType, marketType);
}
...
async function refreshSnapshotUncached({
  ...
  const snapshot: RuntimeVolume24hSnapshot = {
    exchangeType,
    marketType,
    t: Date.now(),
...
async function refreshSnapshot(params: {
  ...
  const snapshot = await runtimePublicMarketCache.value.getOrLoad({
...
export default runtimeMarketVolume;
export { runtimeMarketVolume };
