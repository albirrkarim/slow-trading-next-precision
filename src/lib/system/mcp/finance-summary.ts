...
interface RuntimeMcpFinancePosition {
  closed?: {
    feeUsdt?: number;
    t?: number;
  };
...
export interface RuntimeMcpFinanceDailyPoint {
...
export interface RuntimeMcpFinanceSummary {
  closedTradeCount: number;
  currency: "USDT";
  daily: RuntimeMcpFinanceDailyPoint[];
...
function knownFees(position: RuntimeMcpFinancePosition): number {
...
function createSummary(params: {
  ...
  instanceName: string;
  mode: RuntimeMode;
  positions: readonly RuntimeMcpFinancePosition[];
  start: string;
}): RuntimeMcpFinanceSummary {
  const range = resolveRange(params.start, params.end);
  const daily = new Map<string, RuntimeMcpFinanceDailyPoint>();
...
const runtimeMcpFinanceSummary = {
...
export default runtimeMcpFinanceSummary;
