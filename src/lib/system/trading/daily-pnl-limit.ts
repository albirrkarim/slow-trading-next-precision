...
export interface RuntimeDailyPnlLimitPosition {
  closed?: { t?: number };
  opened: { t: number };
  pnl: { netUsdt?: number };
}

export interface RuntimeDailyPnlLimitEvaluation {
  day: string;
  pnlUsdt: number;
  reached: boolean;
...
function sumForUtcDay(
  positions: RuntimeDailyPnlLimitPosition[],
  day: string,
): number {
...
function describe(evaluation: RuntimeDailyPnlLimitEvaluation): string {
  return (
...
export default runtimeDailyPnlLimit;
export { runtimeDailyPnlLimit };
