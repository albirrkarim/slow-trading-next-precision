import type { RuntimeEffectiveConfig } from "../runtime/types";
...
export interface RuntimeEntrySequenceCount {
  long: number;
  short: number;
  symbol: string;
  total: number;
...
export interface RuntimeWorkerNeededPoint {
  t: number;
  v: number;
}

export interface RuntimeWorkerNeededEstimate {
  metrics: {
    avg: number;
    max: number;
    min: number;
  };
  points: RuntimeWorkerNeededPoint[];
}

export interface RuntimeEntrySequenceInterval {
  endTimeMs: number;
...
export interface RuntimeSystemCapacitySequence {
...
export interface RuntimeSystemCapacityEstimate {
  capitalPoints: RuntimeWorkerNeededPoint[];
  metrics: {
    avgWorkers: number;
  ...
  sequences: RuntimeSystemCapacitySequence[];
  workerPoints: RuntimeWorkerNeededPoint[];
}
...
function collectEntrySequenceIntervals({
  ...
  const intervals: RuntimeEntrySequenceInterval[] = [];
...
function countEntrySequences({
  ...
  const resultBySymbol = new Map<string, RuntimeEntrySequenceCount>();
  const result: RuntimeEntrySequenceCount[] = [];
...
function makeEmptyWorkerNeededEstimate(): RuntimeWorkerNeededEstimate {
  return {
...
function pushWorkerNeededPoint(
  points: RuntimeWorkerNeededPoint[],
  point: RuntimeWorkerNeededPoint,
) {
...
function makeEmptySystemCapacityEstimate(): RuntimeSystemCapacityEstimate {
  return {
    capitalPoints: [],
    metrics: {
      avgWorkers: 0,
...
function estimateWorkerNeeded({
  ...
  const points: RuntimeWorkerNeededPoint[] = [];
  let activeWorkers = 0;
  let area = 0;
  let max = 0;
...
function isIntervalEligibleForTradingMode({
  ...
  config: RuntimeEffectiveConfig;
  interval: RuntimeEntrySequenceInterval;
}) {
...
function buildCapitalEventSeries({
  ...
  endTimeMs: number;
  intervals: RuntimeSystemCapacitySequence[];
  startTimeMs: number;
  ...
  for (const eventTime of eventTimes) {
    activeValue = reserve.money.roundUsdt(
      activeValue + (eventDeltaByTime.get(eventTime) ?? 0),
    );
...
function estimateSystemMaximalCapacity({
  ...
  const sequences: RuntimeSystemCapacitySequence[] = [];
  ...
  for (const interval of intervals) {
    ...
    const entryMarginUsdt = reserve.entry.adjustMarginForConfig({
    ...
    const maxProfitPct =
      Number.isFinite(takeProfitPct) && takeProfitPct > 0
        ...
        : 0;
    const maxProfitUsdt = reserve.money.roundUsdt(
      entryMarginUsdt * (maxProfitPct / 100),
    );
      ...
      maxProfitPct:
        maxEffectiveCapitalUsdt > 0
          ? reserve.money.roundUsdt(
              (maxProfitUsdt / maxEffectiveCapitalUsdt) * 100,
            )
          : 0,
      maxProfitUsdt,
...
export default runtimeEntrySequences;
export { runtimeEntrySequences };
